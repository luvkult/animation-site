...
export function AboutWrapper({}) {
  const [text, setText] = useState({
    main: "Проекты",
    para: `Создаём 2D/3D анимацию, моушн‑дизайн и объясняющие ролики для брендов и агентств. Полный цикл: концепция, стиль, сториборд, анимация и звук.`,
  });
  ...
  useEffect(() => {
    if (!isDesktop()) {
      setText({
        main: "Проекты",
        para: `2D/3D анимация, моушн‑дизайн и ролики под ключ для брендов и студий.`,
      });
    }
    
    // Восстанавливаем исходные цвета при размонтировании компонента
    return () => {
      resetColors();
    };
  }, []);
    ...
    <main className="flex h-full w-full max-w-maxWidth grow flex-col justify-center text-[5.8vw] md:text-[clamp(20px,_1vw_+_14px,_32px)]  ">
      <div className="anime relative flex flex-col gap-[1em] md:flex-row-reverse md:gap-[2em] ">
        ...
        <Magentic
          ...
          onClick={() => {
            // Сбрасываем цвета перед переходом
            resetColors();
          }}
