...
const FullpageProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const onLeave = (origin: any, destination?: any, direction?: any) => {
    ...
    if (direction == "down") {
      ...
      // Анимируем все элементы одновременно (без stagger)
      tl.from(`.${destination.anchor} .anime`, {
          duration: 0.3,
        })
        .fromTo(
          ...
          opacity: 1,
          duration: 1.1,
          stagger: 0, // Все элементы одновременно
          ease: CustomEase.create("custom", "M0,0 C0.52,0.01 0.16,1 1,1 "),
        },
      )
      // Анимируем псевдоэлемент стрелочки одновременно с остальными
      .to(
        `.${destination.anchor} .left:before`,
        {
          opacity: 1,
            duration: 1.1,
            ease: CustomEase.create("custom", "M0,0 C0.52,0.01 0.16,1 1,1 "),
          },
        "-=1.1", // Начинается одновременно с предыдущей анимацией
        );
    } else {
      gsap
        .timeline()
  ...
  const animateSection = (anchor: string, direction: "down" | "up" = "down") => {
    var flex = screen.width > 540 ? 17 : 5;
    if (direction == "down") {
      gsap
        .timeline()
        .from(`.${anchor} .rounded__div__down`, {
          duration: 0.1,
        })
        .fromTo(
      ...
      // Анимируем стрелочку через CSS переменную (GSAP не может анимировать псевдоэлементы напрямую)
      .to(
        `.${anchor} .left`,
        {
          "--arrow-opacity": 1,
  ...
  const afterLoad = function (origin: any, destination: any, direction: any) {
    ...
    if (destination.anchor === "first" && direction === "none") {
      animateSection("first", "down");
    }
  };
  ...
  useLayoutEffect(() => {
    // Устанавливаем начальное состояние элементов первой секции только на главной странице
    if (pathname === "/") {
      // Начальное состояние уже установлено через CSS
      // Настраиваем rounded div для первой секции
      var flex = screen.width > 540 ? 17 : 5;
      gsap.set(".first .anime", {
        y: "30vh",
        opacity: 0,
      });
      gsap.set(".first .rounded__div__down", {
        height: `${flex}vh`,
      });
      // Устанавливаем начальное состояние стрелочки через CSS переменную
      gsap.set(".first .left", {
        "--arrow-opacity": 0,
      });
    }
  ...
  useEffect(() => {
    // Запускаем анимацию для первой секции только если мы на главной странице
    if (pathname === "/") {
