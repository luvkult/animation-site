import React, { memo, use, useEffect, useRef, forwardRef } from "react";
import { gsap } from "gsap";
import Link from "next/link";

import { cn, isDesktop } from "@/lib/utils";

const LinkWithRef = forwardRef<HTMLAnchorElement, React.ComponentProps<typeof Link>>(
...
const Magentic = ({
  ...
  const magnet = useRef<HTMLAnchorElement | HTMLElement>(null);
  ...
  useEffect(() => {
    ...
    const magnetButton = magnet.current as HTMLElement;
  ...
  const isExternalLink = rest.href?.startsWith("http") || rest.href?.startsWith("mailto:") || rest.href?.startsWith("#") || !rest.href;
  ...
  if (isExternalLink) {
    return (
      <a ref={magnet as React.RefObject<HTMLAnchorElement>} {...commonProps} {...rest}>
        {children}
      </a>
    );
  }
