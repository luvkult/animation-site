...
export function WorkSection({
  ...
  useEffect(() => {
    ...
    if (!video || !item.videoLink || !section) return;
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            // Когда секция видна - запускаем видео немедленно
            playVideo();
            ...
            // Когда секция не видна - ставим на паузу
            video.pause();
          }
        });
      },
      {
        threshold: 0.01, // Секция считается видимой когда 1% на экране
    ...
    // Пытаемся запустить сразу если видео уже загружено
    const tryPlay = () => {
      if (video.readyState >= 2) {
        playVideo();
      }
        ...
        >
          <a
            className={`image image--works image--works${
              index + 1
            } anime rounded-3xl overflow-hidden`}
            target="_blank"
            href={item.link}
          >
          ...
          <div className="title ">
            ...
            <div className="btn-wrap js-letter anime">
              <Magentic
                ...
                scrambleParams={{ text: "Связаться", chars: "-x" }}
              >
                <p className="shapka">
                  <span className="scrambleText">Связаться</span>
                  <svg
